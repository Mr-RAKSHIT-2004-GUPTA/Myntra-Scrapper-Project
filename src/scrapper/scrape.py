...
class ScrapeReviews:
    ...
    def extract_products(self, product_reviews: list):
        try:
            ...
            # ✅ Ensure user_rating is always initialized
            user_rating = []
            user_comment = []
            user_name = []

            for i in review:
                rating = i.findAll("div", {"class": "user-review-main user-review-showRating"})
                comment = i.findAll("div", {"class": "user-review-reviewTextWrapper"})
                name = i.findAll("div", {"class": "user-review-left"})

                if rating:
                    user_rating.extend(rating)
                if comment:
                    user_comment.extend(comment)
                if name:
                    user_name.extend(name)

            # ✅ Ensure user_rating has a default value if empty
            if not user_rating:
                print("⚠ No ratings found, setting default value")
    ...
    def get_review_data(self) -> pd.DataFrame:
        try:
            ...
            data = pd.concat(product_details, axis=0)#vertically stacks the all dataframe
